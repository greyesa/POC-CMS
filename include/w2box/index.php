...
function authorize($silent=false){
	...
	if (!$auth){
		...
		($_SESSION['login'] == $config['admin_username'] && $_SESSION['password']==$config['admin_password'])) {
...
<h1>&nbsp;&nbsp;POC-CMS <br> &nbsp;&nbsp;Sistema de Archivos</h1>
...
$modo = $_GET["modo"];
if($modo=="cerrarnoti"){
 echo '<center>[ <a href="javascript:opener.location.reload(true); self.close();">Cerrar esta ventana</a> ]</center>'; 
...
<?php $sid = md5(uniqid(rand())); //unique file id ?>
 <form method="post" enctype="multipart/form-data" action="">
  ...
  <font size="2"><span id="allowed"><?php echo $lang['filetypesallowed'] ?>: <?php echo join(",",$config['allowed_ext']); ?></span>
...
<iframe name="upload_iframe" style="border: 0;width: 0px;height: 0px;display:none;"></iframe>
<?php } ?>
</div>
<?php } ?>
<div id="filelisting">
...
$direccion = substr (''.$config['storage_path'].'/'.rawurlencode($dlink).'', 6);
